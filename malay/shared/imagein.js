/* =========================================================
   /malay/shared/imagein.js
   Real-photograph input for the stimulus-based modes.

   Why this exists: PSLE Bahasa Melayu Kertas 4 plays the student an actual VIDEO
   for Perbualan Berasaskan Rangsangan. We cannot play video here, but
   a real still image is far closer to the task than a written
   description, because the skill is noticing. Practising
...
(function (global) {
  ...
  function fromFile(file) {
    return new Promise((resolve, reject) => {
      if (!file) { reject(new Error('Tiada gambar dipilih.')); return; }
      if (!/^image\//.test(file.type) && !/\.(jpe?g|png|webp|heic|heif)$/i.test(file.name || '')) {
        reject(Object.assign(new Error('Fail itu bukan gambar. Pilih JPG, PNG atau WEBP.'), { code: 'NOT_IMAGE' }));
        return;
      }
  ...
  function mount(host, opts) {
    ...
    host.innerHTML = `
      <div class="imgin" id="${id}" style="background:var(--card);border:1px dashed var(--border);border-radius:12px;padding:1rem 1.15rem;">
        <div style="display:flex;align-items:baseline;gap:.55rem;flex-wrap:wrap;margin-bottom:.4rem;">
          <span style="font-size:.66rem;color:var(--accent);text-transform:uppercase;letter-spacing:.07em;font-weight:800;">📷 ${esc(label || 'Gambar anda')}</span>
        </div>
        <p style="font-size:.83rem;color:var(--muted);line-height:1.6;margin-bottom:.7rem;">
          ${esc(hint || 'Gunakan gambar sebenar. Ambil gambar sekarang, pilih daripada peranti ini, seret masuk, atau tampal.')}
        </p>
        <div style="display:flex;gap:.5rem;flex-wrap:wrap;align-items:center;">
          <button type="button" class="btn btn-primary" data-act="camera" style="font-size:.8rem;padding:.42rem .85rem;">Ambil gambar</button>
          <button type="button" class="btn btn-secondary" data-act="file" style="font-size:.8rem;padding:.42rem .85rem;">Pilih gambar</button>
    ...
    function fail(e) {
      ...
      status.textContent = (e && e.message) ? e.message : 'Gambar itu tidak dapat digunakan.';
    }
    ...
    function accept(pic) {
      ...
      preview.innerHTML =
        '<img src="' + pic.url + '" alt="Gambar yang anda pilih" ' +
