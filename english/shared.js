/* =========================================================
   PSLE English Studio — Shared Utilities
   Progress · Streak · Tabs · Glossary · Toast · Quiz · MindMap · AI
   Ported from /malay/shared.js with the english_* namespace and
...
const Progress = {
  ...
  recordDrillUsed(topicId) { this.set(topicId, { drillUsed: true }); },

  recordMindMap(topicId) { this.set(topicId, { mindMapSaved: true }); },
...
// ── Streak (14-day widget, legacy shape) ───────────────────
const Streak = {
  META_KEY: 'english_meta',
  ...
  record() {
    ...
    if (milestones.includes(m.streak)) {
      setTimeout(() => showToast(`🔥 ${m.streak} days in a row! Nicely done.`), 400);
    }
  },
...
const AIConfig = {
  ...
  // Per-topic adaptive level (4–6 for PSLE English; 4 is foundational, 6 is exam-pitch).
...
function showAISetup() {
    ...
    <div class="ai-modal">
      <div class="ai-modal-header">
        ...
        <div class="ai-modal-title-block">
          <h3>Set Up AI Coach &amp; Quizzes</h3>
          <p>Google Gemini · Free · Stored only in your browser · Shared with Science and Malay Studios</p>
        </div>
      ...
      <div class="ai-guide-box">
        ...
        <ol class="ai-guide-steps">
          ...
          <li>Copy the key shown — it will start with <code>AIza</code>.</li>
  ...
  document.getElementById('ai-key-save').addEventListener('click', () => {
    ...
    AIConfig.setKey(key);
    statusEl.innerHTML = '<span class="ai-status-ok">✅ Key saved. AI features are now active.</span>';
  ...
  document.getElementById('ai-key-test').addEventListener('click', async () => {
    ...
    try {
      ...
      if (r.ok) {
        statusEl.innerHTML = '<span class="ai-status-ok">✅ Key works. Click Save to activate.</span>';
      } else {
        const err = await r.json().catch(() => ({}));
        const msg = err.error?.message || '';
        if (msg.toLowerCase().includes('api key') || msg.toLowerCase().includes('invalid')) {
          statusEl.innerHTML = '<span class="ai-status-error">❌ Key not recognised — check you copied it fully (step 3 above).</span>';
...
class QuizEngine {
  ...
  answer(index) {
    ...
    nextBtn.textContent = this.current + 1 < this.questions.length ? 'Next →' : 'See result →';
    ...
    return `
      <div style="margin-top:1.5rem;text-align:left">
        ...
        ${this.wrong.map(({ q, chosen }) => `
          <div style="margin-bottom:1.1rem;padding:.85rem;background:var(--surface2);border-radius:8px;border-left:3px solid var(--composition)">
            ...
            <p style="font-size:.8rem;color:#34d399;margin-bottom:.45rem">✓ Correct answer: ${q.options[q.correct]}</p>
  ...
  showResult() {
    ...
    Streak.record();
    if (window.EnglishStreak && typeof window.EnglishStreak.record === 'function') { try { window.EnglishStreak.record(); } catch {} }
    const isNewPB = prevBest !== null && pct > prevBest;

    let msg, colour;
    if (pct === 100) { msg = '🏆 Perfect. You have this one.'; colour = 'var(--grammar)'; }
    else if (pct >= 70) { msg = '🎉 Strong work. Review the missed ones to lock it in.'; colour = 'var(--comprehension)'; }
    else { msg = '📚 Keep going. Review the Learn tab, then try again.'; colour = 'var(--composition)'; }
    ...
    this.container.innerHTML = `
      <div style="text-align:center;padding:2rem 1rem 1rem">
        ...
        <div style="display:flex;gap:.75rem;justify-content:center;flex-wrap:wrap">
          ...
          ${hasWrong ? `<button class="btn btn-primary" id="retry-wrong-btn">🎯 Retry ${this.wrong.length} missed</button>` : ''}
        </div>
      </div>
...
// ── LLM Quiz Engine (Gemini-powered) ───────────────────────
class LLMQuizEngine {
      ...
      if (this.staticFallback.length) {
        this.container.innerHTML = `
          <div class="feedback-box show" style="background:#1a2a1a;border-color:#34d399;margin-bottom:.75rem;font-size:.8rem">
            ⚠️ AI unavailable — using built-in questions.
            <button onclick="showAISetup()" style="margin-left:.5rem;background:none;border:none;color:var(--accent);cursor:pointer;font-size:.78rem">Set up API key ↗</button>
          </div>
        `;
        ...
        this.container.innerHTML = `
          <div class="feedback-box show feedback-wrong">
            ...
            <br><button class="btn btn-ghost" onclick="showAISetup()" style="margin-top:.75rem;font-size:.8rem">⚙️ Set up API key</button>
          </div>
        `;
      }
    }
  ...
  async _generate(count) {
    ...
    const levelDesc = {
      4: 'Primary 4 (age 10) — recall simple concepts and basic examples',
      5: 'Primary 5 (age 11) — apply rules and grammar in familiar situations',
      6: 'Primary 6 PSLE (age 12) — PSLE-pitched questions: analyse, compare, use precise vocabulary'
    }[this.level] || 'Primary 5';

    const prompt = `You are an experienced Singapore MOE English Language teacher preparing students for PSLE.
Generate exactly ${count} multiple-choice questions on "${this.topicName}" for a ${levelDesc} learner.

Rules:
- Each question must have exactly 4 options, with exactly 1 correct answer (index 0–3).
- Aligned with the MOE Primary English Language Syllabus 2020 and the 2025+ PSLE format.
- Vary question types: recall, application, sentence completion, error spotting.
- Include a concise explanation (1-2 sentences) in clear English. The explanation may reference grammar or vocabulary terms.

Reply with ONLY a JSON array — no markdown, no code fences, no extra text:
[{"question":"…","options":["A","B","C","D"],"correct":0,"explanation":"…"}]`;
  ...
  _showResult() {
    ...
    if (pct >= 80 && lv < 6) {
      ...
      setTimeout(() => showToast(`⬆️ Next AI questions will pitch up to P${lv + 1}.`), 600);
    } else if (pct < 45 && lv > 4) {
      AIConfig.setLevel(this.topicId, lv - 1);
      setTimeout(() => showToast(`📖 Next AI questions will ease to P${lv - 1} — keep at it.`), 600);
    }

    let msg, colour;
    if (pct === 100) { msg = '🏆 Perfect.'; colour = 'var(--grammar)'; }
    else if (pct >= 70) { msg = '🎉 Strong. One more pass for full marks.'; colour = 'var(--comprehension)'; }
    else { msg = '📚 Open the Learn tab, then come back.'; colour = 'var(--composition)'; }
    ...
    this.container.innerHTML = `
      <div style="text-align:center;padding:2rem 1rem 1rem">
        ...
        <div style="display:flex;gap:.75rem;justify-content:center;flex-wrap:wrap">
          ...
          ${hasWrong ? `<button class="btn btn-primary" id="retry-wrong-ai-btn">🎯 Retry ${this.wrong.length} missed</button>` : ''}
          <button class="btn btn-ghost" onclick="location.reload()">↺ Reset</button>
        </div>
        <p style="font-size:.72rem;color:var(--muted);margin-top:1rem">Next batch: P${this.level}</p>
      </div>
      ${reviewHtml}
    `;
...
function initAIQuizToggle(staticQuestions = []) {
  ...
  toggleBar.innerHTML = `
    <button class="quiz-mode-btn active" data-mode="static">📋 Static Quiz</button>
    <button class="quiz-mode-btn" data-mode="ai">🤖 AI Quiz${AIConfig.hasKey() ? '' : ' <span class="ai-setup-hint">— set up API key</span>'}</button>
  `;
  ...
  document.addEventListener('jgl:gemini-key-changed', () => {
    ...
    if (!aiBtn) return;
    aiBtn.innerHTML = '🤖 AI Quiz' + (AIConfig.hasKey() ? '' : ' <span class="ai-setup-hint">— set up API key</span>');
  });
  ...
  function showStatic() {
    ...
    if (staticQuestions.length) {
      new QuizEngine(staticQuestions, topicId, quizArea, 0);
    } else {
      quizArea.innerHTML = '<p style="color:var(--muted)">No built-in questions for this topic yet.</p>';
    }
  }
    ...
    if (!AIConfig.hasKey()) {
      quizArea.innerHTML = `
        <div style="text-align:center;padding:2.5rem 1rem">
          ...
          <h3 style="margin-bottom:.5rem">AI Quiz</h3>
          <p style="font-size:.85rem;color:var(--muted);margin-bottom:1.25rem">
            Get fresh, adaptive questions generated by Google Gemini.<br>
            Free API key — under 1 minute to set up.
          </p>
          <button class="btn btn-primary" onclick="showAISetup()">⚙️ Set up API key</button>
        </div>
      `;
      return;
    }
    quizArea.innerHTML = '';
    const engine = new LLMQuizEngine(topicId, topicName, level, quizArea, staticQuestions);
    engine.start(6);
  }
